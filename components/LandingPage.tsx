    ...
    <div className="mt-32 relative max-w-5xl mx-auto space-y-8 animate-in fade-in duration-1000 delay-500">
      ...
      <div className="relative border border-[var(--border)] rounded-[3rem] bg-[var(--surface-muted)] overflow-hidden aspect-[16/8] shadow-2xl shadow-black/[0.02] group">
        ...
        <div className={`absolute inset-0 transition-opacity duration-700 ${activeSlide === 0 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
           <div className="p-10 h-full flex flex-col gap-8">
              ...
              <div className="grid grid-cols-3 gap-6 flex-1">
                 <div className="bg-[var(--surface)] rounded-3xl border border-[var(--border)] p-6 flex flex-col justify-between shadow-[var(--shadow)]">
                    <div className="h-3 w-12 bg-blue-50 rounded-full"></div>
                    <div className="flex items-end gap-2">
                       <span className="text-3xl font-black text-[var(--text-primary)]">82%</span>
                       <span className="text-[10px] font-bold text-green-500 mb-1">+12%</span>
                    </div>
                 </div>
                 <div className="bg-[var(--surface)] rounded-3xl border border-[var(--border)] p-6 flex flex-col justify-between">
                    <div className="h-3 w-16 bg-indigo-50 rounded-full"></div>
                    <div className="h-10 w-full bg-[var(--surface-muted)] rounded-xl"></div>
                 </div>
                 <div className="bg-slate-900 rounded-3xl p-6 flex flex-col justify-between shadow-xl">
                    <Sparkles className="h-6 w-6 text-blue-400" />
                    <div className="space-y-2">
                       <div className="h-2 w-full bg-[var(--surface)]/20 rounded-full"></div>
                       <div className="h-2 w-2/3 bg-[var(--surface)]/10 rounded-full"></div>
                    </div>
                 </div>
              </div>
              <div className="h-24 bg-[var(--surface)] rounded-3xl border border-[var(--border)] p-6 flex items-center justify-between">
                 <div className="flex items-center gap-4">
        ...
        <div className={`absolute inset-0 transition-opacity duration-700 ${activeSlide === 1 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
           <div className="p-10 h-full flex flex-col">
              ...
              <div className="grid grid-cols-7 gap-4 flex-1">
                 {[...Array(14)].map((_, i) => (
                   <div key={i} className="bg-[var(--surface)] rounded-2xl border border-[var(--border)] p-4 relative group/day">
        ...
        <div className={`absolute inset-0 transition-opacity duration-700 ${activeSlide === 2 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
           <div className="p-10 h-full flex gap-10">
              <div className="w-1/3 space-y-4">
                 {[...Array(4)].map((_, i) => (
                   <div key={i} className={`p-4 rounded-2xl border transition-all ${i === 0 ? 'bg-[var(--surface)] border-blue-200 shadow-lg' : 'bg-transparent border-[var(--border)]'}`}>
                      <div className="h-2 w-20 bg-slate-200 rounded-full mb-2"></div>
                      <div className="h-1.5 w-full bg-slate-100 rounded-full"></div>
                   </div>
                 ))}
              </div>
              <div className="flex-1 bg-[var(--surface)] rounded-[2rem] border border-[var(--border)] p-8 shadow-2xl relative">
                 <div className="space-y-6">
                    <div className="h-4 w-32 bg-slate-200 rounded-full mx-auto mb-10"></div>
                    <div className="space-y-3">
                       {[...Array(8)].map((_, i) => (
                         <div key={i} className="h-2 w-full bg-[var(--surface-muted)] rounded-full" style={{ width: `${100 - (i * 3)}%` }}></div>
                       ))}
                    </div>
        ...
        <div className="absolute inset-0 bg-blue-600/0 hover:bg-blue-600/5 transition-all flex items-center justify-center group/overlay cursor-pointer">
           <div className="p-4 bg-[var(--surface)] rounded-full shadow-2xl scale-0 group-hover/overlay:scale-100 transition-all duration-300">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-8 pt-8">
        <div className="space-y-2 text-center md:text-left transition-all duration-500">
           ...
           <p className="text-[var(--text-muted)] font-medium">{slides[activeSlide].desc}</p>
        </div>
...
const FAQItem: React.FC<{ question: string; answer: string }> = ({ question, answer }) => {
  ...
  return (
    <div className="border-b border-[var(--border)] last:border-0 group">
      ...
      >
        <span className={`text-base font-medium transition-all ${isOpen ? 'text-[var(--text-primary)]' : 'text-[var(--text-muted)] group-hover:text-[var(--text-primary)]'}`}>{question}</span>
        <div className={`transition-transform duration-300 ${isOpen ? 'rotate-180 text-blue-500' : 'text-[var(--text-muted)]'}`}>
          <ChevronDown className="h-4 w-4" />
        </div>
      </button>
      {isOpen && (
        <div className="pb-7 animate-in fade-in duration-700">
          <p className="text-[var(--text-muted)] leading-relaxed max-w-2xl text-sm">{answer}</p>
        </div>
      )}
    </div>
  );
...
export const LandingPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] text-[var(--text-muted)] selection:bg-blue-500/30 selection:text-white">
      ...
      <nav className="fixed top-0 left-0 right-0 z-[100] py-6 md:py-10 transition-all bg-[var(--background)] md:bg-transparent backdrop-blur-md md:backdrop-blur-none border-b border-[var(--border)] md:border-0">
        <div className="max-w-6xl mx-auto px-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
             ...
             <span className="font-bold text-xl md:text-2xl text-[var(--text-primary)] tracking-tighter">Guardião</span>
          </div>
          ...
          <div className="hidden md:flex items-center gap-12 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--text-muted)]/60 transition-colors">
        ...
        <div className="max-w-4xl mx-auto text-center space-y-12">
          <div className="space-y-6">
            <h1 className="text-5xl md:text-7xl font-black text-[var(--text-primary)] leading-[1.1] tracking-tight animate-in fade-in slide-in-from-bottom-4 duration-1000">
              ...
              <span className="text-[var(--text-muted)]/50 italic font-medium">sem complexidade.</span>
            </h1>
            <p className="text-lg md:text-xl text-[var(--text-muted)] max-w-2xl mx-auto leading-relaxed font-light animate-in fade-in slide-in-from-bottom-4 delay-200 duration-1000">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 pt-4 animate-in fade-in slide-in-from-bottom-4 delay-500 duration-1000">
            ...
            <a href="#recursos" className="text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors bg-[var(--surface-muted)] px-8 py-5 rounded-2xl border border-[var(--border)]">
              Explorar Recursos
            </a>
          </div>
            ...
            {features.map((f, i) => (
              <div key={i} className="space-y-6 group">
                <div className="p-4 bg-[var(--surface-muted)] border border-[var(--border)] rounded-2xl w-fit group-hover:bg-blue-600 transition-all duration-500 group-hover:shadow-xl group-hover:shadow-blue-600/20 group-hover:-translate-y-1">
                  <f.icon className="h-5 w-5 text-[var(--text-muted)] group-hover:text-white" />
                </div>
                <div className="space-y-2">
                  <h4 className="text-[var(--text-primary)] font-bold text-base tracking-tight">{f.title}</h4>
                  <p className="text-sm text-[var(--text-muted)] leading-relaxed font-light">{f.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-48 px-6">
        <div className="max-w-3xl mx-auto">
          <div className="space-y-2 mb-16">
             ...
             <p className="text-[var(--text-muted)] font-medium italic">Respostas rápidas para sua jornada inicial.</p>
          </div>
      ...
      <footer className="py-24 px-6 border-t border-[var(--border)]">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12">
          <div className="flex items-center gap-2">
             ...
             <span className="font-bold text-[var(--text-primary)] tracking-tighter">Guardião</span>
          </div>
